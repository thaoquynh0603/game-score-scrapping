...
class ign:
    ...
    def retrieve_score(self):
        time.sleep(5)
        try:
            xpath_ignrating = "/html[1]/body[1]/div[2]/div[1]/main[1]/div[1]/div[1]/section[1]/div[1]/div[2]/div[2]/div[1]/figure[1]/div[1]/span[1]/figcaption[1]"
            ignrating = self.driver.find_element(By.XPATH,xpath_ignrating).text
        except NoSuchElementException:
            self.error.append('unable to get the rating')
            ignrating = np.nan
        try:
            xpath_user = "/html[1]/body[1]/div[2]/div[1]/main[1]/div[1]/div[1]/section[1]/div[1]/div[2]/div[2]/div[3]/div[1]/h3[1]"
            user = self.driver.find_element(By.XPATH,xpath_user).text
        except NoSuchElementException:
            self.error.append('unable to get the user rating')
            user = np.nan
        return ignrating, user
...
def main(x,y):
    ...
    url_list = ["https://www.ign.com/games/"+x for x in game_list]
    game = url_list
    ...
    result.to_csv(f"ign/ign_{x}_{y}.csv")
